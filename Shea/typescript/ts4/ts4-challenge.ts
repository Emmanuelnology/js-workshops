...
export const isVegetarian = (person:Person):boolean => 
  person.isVegetarian;

export const canFitOnANameCard = (person:Person):boolean => 
  person.name.length <= 6;

export const getLifeExpectancy = (person:Person):number => 
  person.isVegetarian ? 90:95;

export const yearsRemaining = (person:Person):number => 
  getLifeExpectancy(person) - person.age;

export const canRideOnARollercoaster = (person:Person):boolean =>
  person.height >= 120;

export const canBuyBeer = (person:Person):boolean => 
  person.age >= 18;

export const getSummary = (person:Person) => {
  const vegetarianMessage = isVegetarian(person) ? " is a vegetarian,":" likes meat,";
  const deadMessage = yearsRemaining(person) < 0 ? " should be dead": " has " + yearsRemaining(person) + " years to live";
