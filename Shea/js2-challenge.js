...
Functions should take a single parameter which is the personID of the person and return the result rather than log to the console.
...
var vegetarian = people[personID][3];
...
function yearsRemaining(personID) {
    ...
    if (isVegetarian(personID) === false) {
        lifeExpectancy = 90;
    }
...
function getSummary(personID) {
    ...
    if (yearsRemaining(personID) <= 0) {
        var deadMessage = "should be dead already";
    } else {
        deadMessage = "has" + yearsRemaining + " years to live";
    }
