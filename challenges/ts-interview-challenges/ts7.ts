...
export function missingNumberInSequence(numberArray:number[]) {
  let sortedArray=nSort(numberArray);
  let matches:number[]=[];
  for (let numIndex = 1; numIndex < sortedArray.length; numIndex++) {
    let currentNumber=sortedArray[numIndex];
    let previousNumber=sortedArray[numIndex-1];
    if((currentNumber - previousNumber) != 1){
      for (let insertedNumber = previousNumber + 1; insertedNumber < currentNumber; insertedNumber++) {
...
export function findDuplicate(numberArray:number[]) {
  ...
  for(let num of numberArray){
    if (matches.indexOf(num) == -1) matches.push(num);
      else return num;
  }
  return undefined;
}
