...
function missingNumberInSequence(numberArray) {
    var sortedArray = nSort(numberArray);
    var matches = [];
    for (var numIndex = 1; numIndex < sortedArray.length; numIndex++) {
        var currentNumber = sortedArray[numIndex];
        var previousNumber = sortedArray[numIndex - 1];
        if ((currentNumber - previousNumber) != 1) {
            for (var insertedNumber = previousNumber + 1; insertedNumber < currentNumber; insertedNumber++) {
                matches.push(insertedNumber);
            }
        }
    }
    return matches;
}
exports.missingNumberInSequence = missingNumberInSequence;
// How do you find the duplicate number on a given integer array?
...
exports.findLargestAndSmallest = findLargestAndSmallest;
// How do you find all pairs of an integer array whose sum is equal to a given number?
